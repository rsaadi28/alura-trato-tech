...
export function* carregarPagamentoSaga() {
    try {
        ...
        const bandeiras = yield call(bandeirasService.buscaPorId, bandeirasIds);
        const cartoesComBandeiras = cartoes.map(cartao => {

            const bandeiraDoCartao = bandeiras.find(bandeira => parseInt(bandeira.id) === cartao.bandeiraId);
